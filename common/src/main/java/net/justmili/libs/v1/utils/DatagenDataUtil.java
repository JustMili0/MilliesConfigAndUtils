...
public class DatagenDataUtil {
    ...
    public static void shapeless(RecipeCategory category, Item output, int outCount, Item... inputs) {
        ...
        builder.unlockedBy(RecipeProvider.getHasName(inputs[0]), RecipeProvider.has(inputs[0]))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(output)+"_from_"+inputNames));
    }
    public static void shapeless(RecipeCategory category, Item output, Item... inputs) {
        shapeless(category, output, 1, inputs);
    }

    public static void shaped2x2(RecipeCategory category, Item material, Item output, int outCount) {
        ShapedRecipeBuilder.shaped(category, output, outCount)
            ...
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(output)));
    }
    public static void shaped3x3(RecipeCategory category, Item material, Item output, int outCount) {
        ShapedRecipeBuilder.shaped(category, output, outCount)
            .define('#', material)
            .pattern("###")
            .pattern("###")
    ...
    public static void planks(Item log, Item planks) {
        ShapelessRecipeBuilder.shapeless(RecipeCategory.BUILDING_BLOCKS, planks, 4)
            .requires(log)
            .unlockedBy(RecipeProvider.getHasName(log), RecipeProvider.has(log))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(planks)));
    }
    public static void stairs(Item material, Item stairs) {
        RecipeProvider.stairBuilder(stairs, Ingredient.of(material))
            .unlockedBy(RecipeProvider.getHasName(material), RecipeProvider.has(material))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(stairs)));
    }
    public static void slab(Item material, Item slab) {
        RecipeProvider.slabBuilder(RecipeCategory.BUILDING_BLOCKS, slab, Ingredient.of(material))
            .unlockedBy(RecipeProvider.getHasName(material), RecipeProvider.has(material))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(slab)));
    }
    public static void fence(Item material, Item fence) {
        RecipeProvider.fenceBuilder(fence, Ingredient.of(material))
            .unlockedBy(RecipeProvider.getHasName(material), RecipeProvider.has(material))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(fence)));
    }
    public static void fenceGate(Item material, Item fenceGate) {
        RecipeProvider.fenceGateBuilder(fenceGate, Ingredient.of(material))
            .unlockedBy(RecipeProvider.getHasName(material), RecipeProvider.has(material))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(fenceGate)));
    }
    public static void door(Item material, Item door) {
        RecipeProvider.doorBuilder(door, Ingredient.of(material))
            .unlockedBy(RecipeProvider.getHasName(material), RecipeProvider.has(material))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(door)));
    }
    public static void trapdoor(Item material, Item trapdoor) {
        RecipeProvider.trapdoorBuilder(trapdoor, Ingredient.of(material))
            .unlockedBy(RecipeProvider.getHasName(material), RecipeProvider.has(material))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(trapdoor)));
    }

    public static void wall(Item material, Item wall) {
        RecipeProvider.wallBuilder(RecipeCategory.BUILDING_BLOCKS, wall, Ingredient.of(material))
            .unlockedBy(RecipeProvider.getHasName(material), RecipeProvider.has(material))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(wall)));
    }

    public static void bars(Item material, Item bars) {
        ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, bars, 16)
            .define('#', material)
            .pattern("###")
            .pattern("###")
            .unlockedBy(RecipeProvider.getHasName(material), RecipeProvider.has(material))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(bars)));
    }

    public static void pressurePlate(Item material, Item pressurePlate) {
    ...
    public static void smelt(Item input, Item output, float exp, int cookingTime) {
        SimpleCookingRecipeBuilder.smelting(Ingredient.of(input), RecipeCategory.MISC, output, exp, cookingTime)
            ...
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(output)+"_from_"+RecipeProvider.getItemName(input)+"_smelting"));
    }

    public static void smelt(Item input, Item output, float exp) {
        smelt(input, output, exp, 200);
    }

    public static void blast(Item input, Item output, float exp, int cookingTime) {
        SimpleCookingRecipeBuilder.blasting(Ingredient.of(input), RecipeCategory.MISC, output, exp, cookingTime)
            .unlockedBy(RecipeProvider.getHasName(input), RecipeProvider.has(input))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(output)+"_from_"+RecipeProvider.getItemName(input)+"_blasting"));
    }

    public static void blast(Item input, Item output, float exp) {
        blast(input, output, exp, 100);
    }

    public static void smoke(Item input, Item output, float exp, int cookingTime) {
        SimpleCookingRecipeBuilder.smoking(Ingredient.of(input), RecipeCategory.MISC, output, exp, cookingTime)
            .unlockedBy(RecipeProvider.getHasName(input), RecipeProvider.has(input))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(output)+"_from_"+RecipeProvider.getItemName(input)+"_smoking"));
    }

    public static void smoke(Item input, Item output, float exp) {
        smoke(input, output, exp, 100);
    }

    public static void cut(Item input, Item output, int resultCount) {
        SingleItemRecipeBuilder.stonecutting(Ingredient.of(input), RecipeCategory.BUILDING_BLOCKS, output, resultCount)
            .unlockedBy(RecipeProvider.getHasName(input), RecipeProvider.has(input))
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(output)+"_from_"+RecipeProvider.getItemName(input)+"_stonecutting"));
    }
    ...
    public static void smithing(Item base, Item addition, Item template, RecipeCategory category, Item result) {
        SmithingTransformRecipeBuilder.smithing(Ingredient.of(template), Ingredient.of(base), Ingredient.of(addition), category, result)
            ...
            .save(writer, CoreLibs.parse(modId, RecipeProvider.getItemName(result)+"_from_"+RecipeProvider.getItemName(base)+"_smithing"));
    }
}