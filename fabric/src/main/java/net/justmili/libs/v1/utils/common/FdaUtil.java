...
public class FdaUtil {
    ...
    public static <T> AttachmentType<T> create(ResourceLocation id, T defaultValue) {
        return AttachmentRegistry.<T>builder().initializer(() -> defaultValue).copyOnDeath().buildAndRegister(id);
    }

    //Creates values that will NOT clear after a restart
    public static <T> AttachmentType<T> createPersistent(ResourceLocation id, T defaultValue, Codec<T> codec) {
        return AttachmentRegistry.<T>builder().initializer(() -> defaultValue).copyOnDeath().persistent(codec).buildAndRegister(id);
    }
}
